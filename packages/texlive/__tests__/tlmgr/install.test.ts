...
import * as ctanApi from '#texlive/ctan/api';
...
describe.each(
  ...
  beforeEach(() => {
    ...
    toTL['shellesc'] = 'tools';
    vi.spyOn(ctanApi, 'pkg').mockResolvedValue(shellesc);
  });
  ...
  test('using the pre-generated dictionary', async () => {
    ...
    expect(ctanApi.pkg).not.toHaveBeenCalled();
  });
  ...
  test('using the CTAN API', async () => {
    ...
    expect(ctanApi.pkg).toHaveBeenCalledWith('shellesc');
  });
});
